...
void RunMotor(int speed, int motor_number, double max_pwm, double min_pwm)
{
  ...
  else if((speed < 0) && (motor_number == 1))
  {
    ...
    drv_pwm_set_duty(RPWM2, 8, (int)((unit_value*speed)+min_pwm));
  }
  else if((speed == 0) && (motor_number == 1))
  {
    digitalWrite(RPWM1, LOW);
  ...
  else if((speed < 0) && (motor_number == 2))
  {
    ...
    drv_pwm_set_duty(LPWM2, 8, (int)((unit_value*speed)+min_pwm));
  }
  else if((speed == 0) && (motor_number == 2))
  {
    digitalWrite(LPWM1, LOW);
...
void setup() {
  ...
  // Motor 0 speed setting
  drv_pwm_set_duty(RPWM1, 8, 0); // 1. pin number   2. 8(bit) is resolution  3. 0~255 
...
void loop() {
  ...
  RunMotor(20, 1, 255, 70); // 1 right 2 left
  RunMotor(20, 2, 255, 70);
